...
- [TECHNICAL]: Verify git write access by running `git fetch upstream` before issue selection or coding; if `.git/FETCH_HEAD` gives Permission denied, halt and fix permissions first ΓÇö branching from a stale base produces a broken submission.
- [TECHNICAL]: Before selecting any issue, confirm the local Qiskit Rust extension builds and imports from current main (`python -c "import qiskit"` must not raise `cannot import name '_accelerate'`) and that `cargo` is on PATH; without a working build, "reproduce before fix" and "run relevant tests" cannot be honestly satisfied.
