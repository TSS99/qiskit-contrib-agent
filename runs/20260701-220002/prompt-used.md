...
It has been at least 1 days since the last agent PR (or there is
none yet), and 2 of 3 agent PRs are open, so you MAY open one
...
- [FEEDBACK]: Demonstrate genuine human understanding in review responses ΓÇö jakelishman stated "just outputting the code is not a bottleneck we are concerned with ΓÇö it's review and understanding"; reply by explaining the chosen fix in your own words, not by restating the reviewer's point verbatim.
- [FEEDBACK]: Ecosystem addon repos vary in receptivity ΓÇö qiskit-addon-cutting merged PR 817 promptly with "LGTM. Thank you", but david-pl (QuEraComputing) silently closed both bloqade-circuit PRs (#800 and #801) on 2026-06-15 with zero comments or review; vet each repo's maintainer posture before contributing rather than assuming all addon repos welcome AI-assisted PRs.
...
- [SELECTION]: Skip issues with a competing open PR ΓÇö verify using `gh pr list --search "issue:<N>"` and keyword queries, not just a visual shortlist check; Stage 1 missed PR #12317 for issue #12062 because the search was superficial, not because the PR was hidden.
...
- [SELECTION]: When a dormant open PR exists and a maintainer has explicitly asked for it to be revived, comment on the existing PR or issue offering to contribute rather than silently opening a duplicate ΓÇö duplicate PRs are closed on sight and damage contributor reputation.
...
- [TECHNICAL]: When `reno lint` is not installed locally, manually parse the release note YAML for required fields (`fixes:` or `features:` key, valid YAML structure) and run `python tools/find_stray_release_notes.py` as a substitute; do not claim release note validation passed without one of these checks.
...
- [PRSTYLE]: Always name the actual tool used in the AI disclosure, not a placeholder or assumed tool name ΓÇö Stage 1 of run 20260629 initially named "OpenAI Codex (GPT-5)" when the actual tooling was Claude Code; an incorrect disclosure is a honesty failure and can be flagged by maintainers.
